...
import (
	...
	client "github.com/utkarshmani1997/jiva-operator/pkg/kubernetes/client/v1alpha1"
...
// kubernetes cluster
type rolloutStatusFn func(d *appsv1.StatefulSet) (*RolloutOutput, error)

// rolloutStatusfFn is a typed function that abstracts
// fetching rollout status of a deployment instance from
// kubernetes cluster
type rolloutStatusfFn func(d *appsv1.StatefulSet) ([]byte, error)
...
func defaultGet(
	...
	return cli.AppsV1().StatefulSets(namespace).Get(name, *opts)
}
...
func defaultList(
	...
	return cli.AppsV1().StatefulSets(namespace).List(*opts)
}
...
func defaultCreate(
	...
	return cli.AppsV1().StatefulSets(namespace).Create(deploy)
}
...
func defaultDel(
	...
	return cli.AppsV1().StatefulSets(namespace).Delete(name, opts)
}
...
func defaultPatch(
	...
	return cli.AppsV1().StatefulSets(namespace).Patch(name, pt, data, subresources...)
}

// defaultRolloutStatus is the default implementation to
// fetch rollout status of a deployment instance from kubernetes
// cluster
func defaultRolloutStatus(d *appsv1.StatefulSet) (*RolloutOutput, error) {
	b := NewForAPIObject(d)
...
func defaultRolloutStatusf(d *appsv1.StatefulSet) ([]byte, error) {
	b := NewForAPIObject(d)
...
func (k *Kubeclient) Get(name string) (*appsv1.StatefulSet, error) {
...
func (k *Kubeclient) List(opts *metav1.ListOptions) (*appsv1.StatefulSetList, error) {
	cli, err := k.getClientOrCached()
	if err != nil {
		return nil, err
	}
...
func (k *Kubeclient) Create(deployment *appsv1.StatefulSet) (*appsv1.StatefulSet, error) {
	if deployment == nil {
